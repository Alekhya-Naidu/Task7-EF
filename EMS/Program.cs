...
{   
    private static readonly AlekhyaEmployeeDbContext _context;
    ...
    static Program()
    {
        ...
        _context = new AlekhyaEmployeeDbContext();
        _rolesBAL = new RolesBAL(new RolesDAL(_context));    
        _masterDataBAL = new MasterDataBAL(new MasterDataDAL(_context));
        _employeeBal = new EmployeeBAL(_context, new EmployeeDAL(_context, new MasterDataDAL(_context), new RolesDAL(_context)), _masterDataBAL, _rolesBAL);
    }
    ...
    private static void AddEmployee()
    {
        ...
        catch (Exception ex)    
        {
            ...
            {
                _console.PrintError($"Inner Exception: {ex.InnerException.Message}");
            }
        }
    }
    ...
    private static void UpdateEmployeeDetails(EmployeeDetail employeeDetail)
    {
        try
        {
            if(_employeeBal.Update(employeeDetail) > 0)
            {
    ...
    public static EmployeeDetail GetEmployeeInputs()
    {
        ...
        DateOnly dob = DateOnly.Parse(Console.ReadLine() ?? "0000-00-00");
        ...
        DateOnly joiningDate = DateOnly.Parse(Console.ReadLine() ?? "0000-00-00");
    ...
    public static void FilterAndDisplay()
    {
        ...
        string empNoInput = Console.ReadLine();
        int? empNoFilter = string.IsNullOrWhiteSpace(empNoInput) ? null : ParseEmployeeId(empNoInput);
    ...
    private static int? ParseEmployeeId(string empNoInput)
    {
        if (int.TryParse(empNoInput, out int empNoFilter))
        {
            return empNoFilter;
        }
        else
        {
            _console.PrintError("Invalid employee number.");
            return null;
        }
    }
    ...
    public static void Update()
    {
        ...
        EmployeeDetail existingEmployee = _employeeBal.GetAllEmployees().FirstOrDefault(emp => emp.Uid == empNo);
        ...
        EmployeeDetail employeeDetail = new EmployeeDetail
        {
            ...
            DepartmentName = department?.Name ?? "0",
            RoleName = role?.Name ?? "0",
            IsManager = isManager,
            ManagerName = managerName,
            ProjectName = project?.Name ?? "0",
        };
        employeeDetail.Id = existingEmployee.Id;
